...
main = do
    ...
    Crit.defaultMain [
        Crit.bgroup "sir-io-atomic-cores"
        [ Crit.bench "51x51"   $ Crit.nfIO (initSim g t dt ( 51,  51)) ]
      , Crit.bgroup "sir-io-atomic-agents"
        [ Crit.bench "51x51"   $ Crit.nfIO (initSim g t dt ( 51,  51))
    ...
    susceptible = proc _ -> do
      ...
      if not $ isEvent makeContact 
        then returnA -< ((), NoEvent)
          ...
          -- READ: also reading has to go through atomicModifyIORef!
          e <- arrM (liftIO . atomicModifyIORef' env) -< (\e' -> (e', e'))
            ...
            Infected -> do
              ...
              if infected 
                then (do
                  ...
                  arrM (liftIO . atomicModifyIORef' env) -< const (e', ())
                  returnA -< ((), Event ()))
  ...
  where
    ...
    infected = proc _ -> do
      ...
      if isEvent recovered
        then (do
          -- WRITE through atomicModifyIORef
          arrM (liftIO . atomicModifyIORef' env) -< (\env' -> (changeCell coord Recovered env', ()))
          returnA -< ((), Event ()))
        else returnA -< ((), NoEvent)
